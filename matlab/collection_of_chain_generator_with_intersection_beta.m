...
% number_of_plates_in_chain = 20;
number_of_chains_to_generate = 10;
...
basic_hex1 = [L/2 -a*sqrt(3)/2 -a/2; L/2 -a*sqrt(3)/2 a/2; L/2 0 a; L/2 a*sqrt(3)/2 a/2; L/2 a*sqrt(3)/2 -a/2; L/2 0 -a; -L/2 -a*sqrt(3)/2 -a/2; -L/2 -a*sqrt(3)/2 a/2; -L/2 0 a; -L/2 a*sqrt(3)/2 a/2; -L/2 a*sqrt(3)/2 -a/2; -L/2 0 -a; ];
a=1./5;
L=2;
% basic_hex2 = [L/2 -a*sqrt(3)/2 -a/2; L/2 -a*sqrt(3)/2 a/2; L/2 0 a; L/2 a*sqrt(3)/2 a/2; L/2 a*sqrt(3)/2 -a/2; L/2 0 -a; -L/2 -a*sqrt(3)/2 -a/2; -L/2 -a*sqrt(3)/2 a/2; -L/2 0 a; -L/2 a*sqrt(3)/2 a/2; -L/2 a*sqrt(3)/2 -a/2; -L/2 0 -a; ];
basic_hex2 = basic_hex1;

basic_hex=basic_hex1;
...
beta_trans = 5; 0.1; % Controls translation angle around z-axis
...
for current_chain = 1:number_of_chains_to_generate
    ...
    for current_hex = 2:number_of_plates_in_chain
        ...
        if ~mod(current_hex,2)
            basic_hex=basic_hex2;
        else
            basic_hex=basic_hex1;
        end
    ...
    for i = 1:number_of_plates_in_chain
        for j = 1:number_of_plates_in_chain
            max_distance_sum_for_aggregate = max_distance_sum_for_aggregate + abs(i-j)*max(2*a,L);
        end
    end
    for i = 1:number_of_plates_in_chain
        for j = 1:number_of_plates_in_chain
